...
describe('Job Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.jobs?.[0]).toEqual(expect.objectContaining({ jobId: 'ABC' }));
  });
  ...
  describe('trackJobId', () => {
    it('Should forward to jobService', () => {
      ...
      const jobId = comp.trackJobId(0, entity);
      expect(service.getJobIdentifier).toHaveBeenCalledWith(entity);
      expect(jobId).toBe(entity.jobId);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['jobId,desc'] }));
  });
