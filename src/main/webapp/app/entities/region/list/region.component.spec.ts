...
describe('Region Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.regions?.[0]).toEqual(expect.objectContaining({ regionId: 123 }));
  });

  describe('trackRegionId', () => {
    it('Should forward to regionService', () => {
      const entity = { regionId: 123 };
      jest.spyOn(service, 'getRegionIdentifier');
      const regionId = comp.trackRegionId(0, entity);
      expect(service.getRegionIdentifier).toHaveBeenCalledWith(entity);
      expect(regionId).toBe(entity.regionId);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['regionId,desc'] }));
  });
