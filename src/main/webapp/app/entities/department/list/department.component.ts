...
export class DepartmentComponent implements OnInit {
  ...
  trackDepartmentId = (_index: number, item: IDepartment): number => this.departmentService.getDepartmentIdentifier(item);

  search(query: string): void {
    if (query && DepartmentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'departmentId';
      this.ascending = true;
    }
    this.page = 1;
