...
describe('Location Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.locations?.[0]).toEqual(expect.objectContaining({ locationId: 123 }));
  });

  describe('trackLocationId', () => {
    it('Should forward to locationService', () => {
      const entity = { locationId: 123 };
      jest.spyOn(service, 'getLocationIdentifier');
      const locationId = comp.trackLocationId(0, entity);
      expect(service.getLocationIdentifier).toHaveBeenCalledWith(entity);
      expect(locationId).toBe(entity.locationId);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['locationId,desc'] }));
  });
