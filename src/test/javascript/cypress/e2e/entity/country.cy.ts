...
describe('Country e2e test', () => {
  ...
  afterEach(() => {
    if (country) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/api/countries/${country.countryId}`,
      }).then(() => {
        country = undefined;
      });
    }
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="countryId"]`).type('d1e7ebf2-c69d-4f90-89b9-fac66970abeb');
      cy.get(`[data-cy="countryId"]`).should('have.value', 'd1e7ebf2-c69d-4f90-89b9-fac66970abeb');

      cy.get(`[data-cy="countryName"]`).type('oh');
      cy.get(`[data-cy="countryName"]`).should('have.value', 'oh');
