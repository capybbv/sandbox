...
describe('Department e2e test', () => {
  ...
  afterEach(() => {
    if (department) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/api/departments/${department.departmentId}`,
      }).then(() => {
  ...
  describe('new Department page', () => {
    ...
    it('should create an instance of Department', () => {
      cy.get(`[data-cy="departmentName"]`).type('mmm after considering');
      cy.get(`[data-cy="departmentName"]`).should('have.value', 'mmm after considering');
