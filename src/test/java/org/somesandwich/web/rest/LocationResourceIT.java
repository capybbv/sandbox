...
class LocationResourceIT {
    ...
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{locationId}";
    ...
    void getAllLocations() throws Exception {
        ...
        restLocationMockMvc
            .perform(get(ENTITY_API_URL + "?sort=locationId,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].locationId").value(hasItem(location.getLocationId().intValue())))
    ...
    void getLocation() throws Exception {
        ...
        // Get the location
        restLocationMockMvc
            .perform(get(ENTITY_API_URL_ID, location.getLocationId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.locationId").value(location.getLocationId().intValue()))
    ...
    void putExistingLocation() throws Exception {
        ...
        // Update the location
        Location updatedLocation = locationRepository.findById(location.getLocationId()).orElseThrow();
        ...
        restLocationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedLocation.getLocationId())
    ...
    void putNonExistingLocation() throws Exception {
        int databaseSizeBeforeUpdate = locationRepository.findAll().size();
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(locationSearchRepository.findAll());
        location.setLocationId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restLocationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, location.getLocationId())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void partialUpdateLocationWithPatch() throws Exception {
        ...
        partialUpdatedLocation.setLocationId(location.getLocationId());

        partialUpdatedLocation.postalCode(UPDATED_POSTAL_CODE).stateProvince(UPDATED_STATE_PROVINCE);

        restLocationMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedLocation.getLocationId())
    ...
    void patchNonExistingLocation() throws Exception {
        ...
        restLocationMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, location.getLocationId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(location))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteLocation() throws Exception {
        ...
        // Delete the location
        restLocationMockMvc
            .perform(delete(ENTITY_API_URL_ID, location.getLocationId()).accept(MediaType.APPLICATION_JSON))
    ...
    void searchLocation() throws Exception {
        ...
        // Search the location
        restLocationMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + location.getLocationId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].locationId").value(hasItem(location.getLocationId().intValue())))
            .andExpect(jsonPath("$.[*].streetAddress").value(hasItem(DEFAULT_STREET_ADDRESS)))
