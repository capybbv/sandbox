...
class RegionResourceIT {
    ...
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{regionId}";
    ...
    void getAllRegions() throws Exception {
        ...
        restRegionMockMvc
            .perform(get(ENTITY_API_URL + "?sort=regionId,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].regionId").value(hasItem(region.getRegionId().intValue())))
    ...
    void getRegion() throws Exception {
        ...
        // Get the region
        restRegionMockMvc
            .perform(get(ENTITY_API_URL_ID, region.getRegionId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.regionId").value(region.getRegionId().intValue()))
    ...
    void putExistingRegion() throws Exception {
        ...
        // Update the region
        Region updatedRegion = regionRepository.findById(region.getRegionId()).orElseThrow();
        ...
        restRegionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedRegion.getRegionId())
    ...
    void putNonExistingRegion() throws Exception {
        int databaseSizeBeforeUpdate = regionRepository.findAll().size();
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(regionSearchRepository.findAll());
        region.setRegionId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restRegionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, region.getRegionId())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void partialUpdateRegionWithPatch() throws Exception {
        ...
        partialUpdatedRegion.setRegionId(region.getRegionId());

        restRegionMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedRegion.getRegionId())
    ...
    void patchNonExistingRegion() throws Exception {
        ...
        restRegionMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, region.getRegionId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(region))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteRegion() throws Exception {
        ...
        // Delete the region
        restRegionMockMvc
            .perform(delete(ENTITY_API_URL_ID, region.getRegionId()).accept(MediaType.APPLICATION_JSON))
    ...
    void searchRegion() throws Exception {
        ...
        // Search the region
        restRegionMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + region.getRegionId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].regionId").value(hasItem(region.getRegionId().intValue())))
            .andExpect(jsonPath("$.[*].regionName").value(hasItem(DEFAULT_REGION_NAME)));
